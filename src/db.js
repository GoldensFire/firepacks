...
for (const [name, definition] of [
	...
	// Сколько в паке чужих вопросов — не тем, а именно вопросов: столько их
	// стоит в заимствованных темах (см. questions у темы в src/siq.js). Доля
	// на этот вопрос не отвечает, а спрашивают именно его. Считается заодно
	// с долей и лежит готовым по той же причине, что и repeat_share: складывать
	// его на лету пришлось бы обходом json_each по всей таблице.
	// NULL — «не считали», ноль — «темы чужие, а числа вопросов у них нет»
	// (так разобраны паки до появления образцов)
...
// Ключ пака — не колонка, а выражение из pack_id и названия (см. PACK_KEY_SQL
// в keys.js). Так и задумано: один и тот же файл нередко выложен в обсуждение
// не по разу, лежит в базе несколькими строками с разными номерами, а отметка
// «сыграно», оценка и чёрный список у него одни на все копии — они про пак,
// а не про строку.
//
// Платой за это был обход всей таблицы на каждый вопрос «а что это за пак».
// Причём обход с подъёмом каждой строки целиком: ключ считается из двух её
...
// D1 (см. getProfile в cf/src/library.js).
//
...
// в cf/src/library.js). Отсюда же и «IF NOT EXISTS» без всяких условий: запрос
