...
/* Отпечатки вопросов пака: по ним ищется списанное (см. src/plagiarism.js).
   ...
   с весом приложенных к вопросу файлов. Хранится рядом, потому что длиной
   свёртки эти два вида не различить: сорок восемь байт делится и на то,
   и на другое (см. PRINTS_VERSION в src/plagiarism.js). */
...
for (const [name, definition] of [
	...
	// измерения, меряется заново (см. fetchDurations в src/indexer.js)
	['media_at', 'INTEGER'],
]) {
