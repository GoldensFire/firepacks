...
//      Тем же правилом отсеиваются области знаний — «История», «География»,
//      «Вторая мировая война»: повторяться может только произведение, а тема
//      про столицы и тема про реки — это не пак, дважды вернувшийся к одному.
...
const FORMAT_WORDS = new Set([
	...
	// Область знаний — не произведение. Повторяться может только произведение:
	// пак, где есть тема про столицы и тема про реки, дважды ни к чему
	// не возвращался, а до этой строки получал повтор «География»
...
/**
 * То же самое, но целыми названиями: областей из нескольких слов, ни одно
 * из которых само по себе служебным не является. «Вторая мировая война» —
...
/**
 * Название целиком состоит из служебных слов (или само целиком — область
 * знаний)? Считаем пометкой, только если пометкой является КАЖДОЕ слово:
 * «Игра престолов» и «История игрушек» — обычные названия, а «Ранобэ
 * иллюстрация» и «История» — нет.
 */
export function isFormatMarker(text) {
	if (NOT_A_WORK.has(normalize(text))) {
		return true;
	}
