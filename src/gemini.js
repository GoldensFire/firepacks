...
- Аниме и манга — разные категории, и решает носитель, а не вселенная. Спрашивают
  ...
  рисовку, мангаку, то, чего в аниме ещё или вовсе не было, — manga. «Наруто»
  бывает и тем, и другим; не видно, о чём именно речь, — бери anime, если
  экранизация есть, и manga, если её нет.
...
export async function analyzePack(pack) {
	...
	const prompt = `${ANALYZE_INSTRUCTION}\n\nПак: «${pack.name ?? ''}»${packTags(pack)}${themeLines(themes)}`;
...
export async function analyzePacks(packs) {
	...
	const list = packs.map((pack, index) => {
		const themes = pack.themes ?? [];
		return `Пак ${index}: «${pack.name ?? ''}»${packTags(pack)}${themeLines(themes)}`;
	});
