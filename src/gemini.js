...
- В w идёт только то, что в теме ОТГАДЫВАЮТ, — верный ответ. Неверные варианты
  и то, что названо с отрицанием, в w не пишутся: в «Угадать другое аниме,
  не Оверлорд» Оверлорд — это как раз не ответ, а в вопросе «В каком аниме нет
  демонов?» перечисленные тайтлы — варианты выбора, а не содержимое пака.
  Такое упоминание накручивает паку повтор, которого в нём нет: человек,
  читающий «пак возвращается к Оверлорду», ждёт вопросов про Оверлорд.
...
const SCHEMA = {
	...
	items: {
		...
		required: ['i', 'c', 'm', 'f', 'fe', 'a', 'ae', 'w', 'k', 'gs', 'gms', 'y', 'og'],
	},
};
	...
	for (const answer of answers) {
		...
		if (theme && EXCLUSIVE_TOPIC_KEYS.includes(answer.c)) {
			result.set(theme.key, {
				...
				genres: cleanCounted(answer.gs, key => isGenre(answer.c, key)),
...
const ANALYZE_SCHEMA = {
	...
	// тысяч паков из четырнадцати
	required: ['s', 'af', 'at', 'mp', 'lg', 't'],
};
