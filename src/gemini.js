...
import {
	config, EXCLUSIVE_TOPIC_KEYS, OTHER_KINDS, OTHER_KIND_KEYS, GENRES, MUSIC_KEY, isGenre,
	LANGUAGE_NAMES, LANGUAGE_KEYS, ORIGIN_KEYS, DECADE_MIN,
} from './config.js';
...
${GENRE_LIST}
У категории other жанров нет: там gs — пустой массив.
...
Ещё два числа: ro — сколько ответов темы про НАШЕ, fo — сколько про зарубежное.
Считается поштучно по тем же ответам.

«Наше» — это русское, русскоязычное, советское и постсоветское: российская
и советская музыка и кино, украинские и белорусские исполнители, мультфильмы
«Союзмультфильма», отечественные игры и книги. «Кино», «Ария», «Океан Ельзи»,
«Иван Васильевич меняет профессию», «Ну, погоди!», «Мастер и Маргарита» — всё
это ro.

«Зарубежное» — всё остальное: американское, британское, японское, корейское,
европейское. Аниме — всегда fo. Голливуд — fo. K-pop — fo.

Решает происхождение произведения, а НЕ язык названия и не язык ответа.
Русская группа с английским именем («Little Big», «Cream Soda», «Molchat Doma»)
— это ro. Английский фильм, у которого есть русское название, — fo.

Ответ, где произведения нет вовсе (мемы, наука, столицы), не считается никуда:
ro и fo складываются не в число ответов темы, а в число тех ответов, про которые
вопрос вообще стоит. Не знаешь — не считай.
...
const SCHEMA = {
	...
	items: {
		...
		properties: {
			...
			// Наше и зарубежное — числом ответов на каждое. Складываются они
			// не в число ответов темы, а в число тех, про которые вопрос вообще
			// стоит: у мема и у вопроса про столицы происхождения нет
			ro: { type: 'INTEGER' },
			fo: { type: 'INTEGER' },
		},
		required: ['i', 'c', 'm', 'f', 'fe', 'a', 'ae', 'w', 'k', 'gs', 'gms', 'y', 'ro', 'fo'],
	},
};
...
const UNMARKED = {
	...
	musicGenres: [],
	decades: [],
	origin: { ru: 0, foreign: 0 },
};

/**
 * Список «ключ — сколько раз» из ответа модели: жанры темы и жанры её музыки.
 * Чужие ключи отсеиваются здесь же — проверять их снаружи было бы поздно.
 *
 * @param {Array} value что ответила модель
 * @param {string} topic из чьего списка брать ключи
 */
function cleanCounted(value, topic) {
	...
	for (const item of value) {
		...
		if (!isGenre(topic, key) || !Number.isFinite(count) || count <= 0) {
			continue;
		}
...
/** Наше и зарубежное числом ответов. Отрицательное и нечисловое — это ноль. */
function cleanOrigin(answer) {
	const count = value => {
		const number = Math.round(Number(value));
		return Number.isFinite(number) && number > 0 ? number : 0;
	};

	return { ru: count(answer?.ro), foreign: count(answer?.fo) };
}
	...
	for (const answer of answers) {
		...
		if (theme && EXCLUSIVE_TOPIC_KEYS.includes(answer.c)) {
			result.set(theme.key, {
				...
				// означал бы, что модель ответила невпопад
				genres: cleanCounted(answer.gs, answer.c),
				// Жанры музыки спрашиваются отдельно и живут отдельно: тема
				// с опенингами — это и аниме (genres), и анисонг (musicGenres)
				musicGenres: answer.m === true ? cleanCounted(answer.gms, MUSIC_KEY) : [],
				// Когда вышло названное в теме и откуда оно родом. Из первого
				// складывается полоска десятилетий, из второго — «наше и зарубежное»
				decades: cleanDecades(answer.y),
				origin: cleanOrigin(answer),
			});
		}
	}
...
export async function describePack(pack) {
	...
	return { summary: cleanSummary(value?.s), audience: cleanAudience(value), language: cleanLanguage(value?.lg) };
}
...
тему, в том же порядке, в каком темы даны.`;
...
export async function analyzePack(pack) {
	...
	try {
		...
		const { summary, audience, language } = await describePack({ ...pack, themes });

		return { marks, summary, audience, language, queries: [], split: true, reason: error.message };
	}
}
...
весь пак, поля af, at и mp — про его аудиторию, поле lg — его язык, поле t —
массив, по объекту на каждую тему этого пака, в том же порядке, в каком темы даны.
