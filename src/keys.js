...
/**
 * Насколько тема, по которой отобрана выдача, — про этот пак.
 *
 * Ярлыки пакам ставит модель, и ставит их щедро: у солянки их полсотни, и футбол
 * среди них стоит рядом с алфавитом, джазом и чгк — потому что одна из тридцати
 * рубрик и вправду про футбол. Отбор по ярлыку это ловит верно, а вот порядок
 * выдачи выходил не тот: страница /tag/futbol открывалась «сначала новыми»,
 * и первыми на ней стояли три солянки, у которых футбол — одна тема из сорока,
 * тогда как «Чокнутый футбольный пак» лежал где-то в середине списка.
 *
 ...
 * так же точно и уже лежит готовым: сколько у пака ярлыков всего. Пак, у которого
 * их два и один из них «Футбол», — футбольный пак; пак, у которого их полсотни, —
 * солянка, где футбол случился. Проверено по базе: у первой дюжины паков,
 * отсортированных так, franchise_top — «Футбол» с долей 0,9–1,0, у последней
 * дюжины ярлыков по сорок семь.
 *
 ...
 * без одной. Пустой ключ даёт ноль, и порядок от этого не портится: пак без
 * ярлыков по ярлыку и не отберётся.
 *
 ...
 * ярлыков.
 */
export const TAG_FIT_SQL = `-(LENGTH(COALESCE(p.tags_key, ''))
	- LENGTH(REPLACE(COALESCE(p.tags_key, ''), '|', '')))`;
