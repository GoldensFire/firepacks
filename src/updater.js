...
export const UPDATE_STEPS = [
	...
	{
		key: 'prints',
		name: 'Снять отпечатки вопросов',
		hint: 'То, по чему ищется списанное: у каждого вопроса берётся восьмибайтный отпечаток его '
			+ 'текста, имени приложенного файла и ответа. Новым пакам его считает разбор, '
			+ 'а этот шаг добирает старые — из архива читается только content.xml, '
			+ 'это два запроса и семь килобайт на пак. Один раз по всей базе — примерно полтора часа; '
			+ 'дальше очередь пуста. Идти должен следом за обходом ВК: ссылки на файлы живут недолго, '
			+ 'и обновляет их только он.',
	},
	{
		key: 'durations',
		name: 'Померить длительность медиа',
		hint: 'Сколько тянется звук и видео в паке: в среднем на файл и самый длинный. '
			+ 'Файлы не качаются — с каждого читается по двенадцать килобайт заголовка, '
			+ 'оттуда и берутся секунды. Но заголовок этот приходится просить отдельным '
			+ 'запросом на файл: составной range-запрос ВК на таких размерах рвёт. '
			+ 'Поэтому шаг самый долгий из всех — минута на пак с полусотней песен, — '
			+ 'и по умолчанию не отмечен. Мерить надо один раз: числа эти не портятся, '
			+ 'а новый пак меряется вместе со своим разбором. Идти должен следом '
			+ 'за обходом ВК: ссылки на файлы живут недолго.',
	},
	{
		key: 'plagiarism',
		name: 'Пересмотреть, что уже встречалось',
		hint: 'Без сети: ищет паки, чьи вопросы уже стояли в чужих паках, выложенных раньше — '
			+ 'по отпечаткам с шага выше. Совпадением считается вопрос целиком: текст, имя файла и ответ; '
			+ 'одинаковые названия тем не значат ничего. Пересматривает всю базу каждый раз, '
			+ 'а не одну очередь, — иначе найденный сегодня старый пак так и остался бы числиться '
			+ 'первоисточником. Считаные секунды.',
	},
	{
		key: 'recalc',
 ...
 * Те же ключи, что у --first= в индексаторе (см. ORDER_NAMES там).
 */
