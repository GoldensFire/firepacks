...
/**
 * Выкладка на Cloudflare прямо из библиотеки: посмотрел на новое — отправил.
 *
 * Сама выкладка идёт на сервере минутами, и следить за ней здесь нечем: лог
 * и полоски живут на странице обновления, туда и уходим, запустив. Спрашиваем
 * перед этим: это единственная кнопка библиотеки, которая что-то меняет
 * за пределами этого компьютера.
 */
async function startDeploy() {
	if (!confirm('Выложить нынешнюю базу и сайт на Cloudflare? Ход выкладки будет виден на странице обновления.')) {
		return;
	}
	...
	try {
		data = await (await fetch('/api/update/deploy', { method: 'POST' })).json();
	} catch (error) {
