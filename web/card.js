 ...
 * за ними жанры внутри тематики, а снизу — из чего пак сделан: текст, картинки,
 * музыка, видео и html.
 *
 * Порядок именно такой: две верхние полоски отвечают на вопрос «про что этот пак»
 * и читаются как одна мысль от общего к частному — тематика, потом жанр внутри
 * неё. Состав же — вопрос совсем другой, про форму, а не про содержание, и стоять
...
function createShares(pack) {
	...
	for (const row of [topics, genres, contents]) {
		if (row) {
			box.append(row);
		}
	}
