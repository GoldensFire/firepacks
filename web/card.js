...
/**
 * Сколько тянется медиа в паке: в среднем на файл и самый длинный.
 *
 * ————— зачем это на карточке —————
 *
 * Число вопросов не отвечает на «сколько играть»: сотня вопросов бывает
 * и на два часа, и на пять. Разницу делает длина куска — тема из шести песен
 * по полторы минуты идёт девять минут одна, — и до сих пор увидеть это можно
 * было только скачав пак.
 *
 * Числа два, потому что вопрос тоже два. Среднее — «сколько тянется вопрос»;
 * самое длинное — «нет ли там целой серии». Средние тридцать секунд при
 * максимуме в пять минут это не пак с короткими отрывками, а пак, в котором
 * где-то лежит полная серия, и её будут смотреть.
 *
 * ————— почему красится только среднее —————
 *
 * Потому что решает именно оно. Один длинный файл — это один длинный вопрос,
 * и он никого не разоряет; а вот средние сорок пять секунд означают, что таков
 * каждый вопрос пака. Порогов два, и они те же, что у повторов
 * и спецвопросов, — общий alarmClass и те же два цвета (см. mediaWarnSeconds
 * в src/settings.js).
 *
 * Пак, у которого медиа нет вовсе, молчит: длительности у картинок не бывает,
 * и «0 с» объявило бы такой пак самым коротким в библиотеке. Молчит и тот,
 * у кого её ещё не мерили, — это разные вещи, и обе одинаково не повод писать
 * ноль.
 */
function createDuration(pack) {
	const media = pack.media;

	if (!media || !media.files || media.average === null || media.average === undefined) {
		return null;
	}

	const alarm = alarmClass(media.average, facets.mediaWarnSeconds, facets.mediaAlarmSeconds);
	const box = element('span', `icon-text meta__duration${alarm}`);

	box.append(
		icon('clock'),
		element('span', 'meta__duration-value', formatSeconds(media.average)),
	);

	// Самое длинное — рядом и бледнее: это уточнение к среднему, а не второе
	// число наравне с ним. Не пишется вовсе, когда файл в паке один: «в среднем
	// минута, самый длинный минута» — это одно и то же, сказанное дважды
	if (media.files > 1 && media.longest > media.average) {
		box.append(element('span', 'meta__duration-longest', `макс. ${formatSeconds(media.longest)}`));
	}

	const measured = media.files < media.total
		? `посчитано по ${media.files} файлам из ${media.total}`
		: `${media.files} ${plural(media.files, 'файл', 'файла', 'файлов')}`;

	box.title = `Медиа в паке: в среднем ${formatSeconds(media.average)} на файл, `
		+ `самый длинный ${formatSeconds(media.longest)} (${measured}). `
		+ 'Считано по заголовкам файлов, без скачивания';
...
/**
 * Метка пака, чьи вопросы уже стояли в чужом паке, выложенном раньше, —
 * со ссылкой на тот пак.
 *
 * ————— почему тут не написано «плагиат» —————
 *
 * Потому что правило этого не знает. Оно говорит ровно одно: такие же вопросы
 ...
 *
 * ————— почему вопросы, а не темы —————
 *
 * Раньше сравнивались темы, и мерка была не по задаче: «Фильмы», «Аниме»,
 * «Разное» стоят в сотнях паков и ничьей собственностью не являются.
 * Собственность — то, что внутри темы. Теперь совпадением считается вопрос
 * целиком: его текст, имя приложенного файла и ответ.
 *
...
function createPlagiarismBadge(pack) {
	...
	// Четыре ступени, и разница между ними не в силе слова, а в том, что
	// утверждается. «Этот пак — копия» и «в этом паке нашлись три чужих
	// вопроса» — разные новости, и выглядеть одинаково они не должны.
	//
	// Нижняя (trace) заведена ради отбора «чужих вопросов хотя бы столько-то»:
	// раз найденное записано, о нём надо и сказать — но сказать тихо, потому
	// что про пак целиком оно не говорит ничего
	const trace = mark.kind === 'trace';
	const partial = mark.kind === 'partial';
	const compiled = mark.kind === 'compiled';
	const soft = trace || partial;

	// Число чужих вопросов — то, ради чего всё и считается: за столом сыграют
	// именно на них. Доля отвечает на соседний вопрос, «какая часть пака чужая»,
	// и одно из другого не выводится: десять чужих вопросов в паке на сотню —
	// это и «десять», и «одна десятая», и спрашивают чаще первое
	const stolen = mark.questions ?? 0;
	const stolenLine = stolen > 0
		? `${formatNumber(stolen)} ${plural(stolen, 'чужой вопрос', 'чужих вопроса', 'чужих вопросов')}`
		: '';
	...
	const badge = element(top ? 'a' : 'span', `badge badge--clue${soft ? ' badge--clue-soft' : ''}`);

	if (top) {
		badge.href = packHref(top);
	}

	// У нижних ступеней доля в подписи стоит, а у верхних — нет, и это
	// не описка. «Копия» либо копия, либо нет, и «96,8%» при ней выглядит
	// точностью, которой у правила нет. А «часть вопросов» без числа не говорит
	// ничего: четверть это или три четверти — весь вопрос.
	//
	// У самой нижней ступени доли нет и в подписи: она там однозначно мала,
	// и «3%» рядом с «два чужих вопроса» было бы третьим числом на ярлыке
	const label = trace
		? 'Есть чужие вопросы'
		: partial
			? `Часть вопросов встречалась раньше · ${percent}%`
			: (compiled ? 'Вопросы из чужих паков' : 'Вопросы встречались раньше');

	badge.append(icon('copy'), element('span', null, stolenLine ? `${label} · ${stolenLine}` : label));

	// Сколько это от пака целиком. В подписи на метке этого нет нарочно: там
	// и так два числа, а третье превратило бы ярлык в строку отчёта. Зато
	// в подсказке оно нужно — «240 чужих вопросов» в паке на 240 и в паке
	// на тысячу это разные новости
	const outOf = pack.questionCount > 0
		? ` из ${formatNumber(pack.questionCount)} вопросов пака`
		: '';

	const caveat = 'Это не обязательно воровство: соавторы выкладывают паки каждый со своей страницы';
	const where = 'Тема, где таких вопросов половина и больше, отмечена ссылкой в «Подробнее»';

	badge.title = trace
		? `${formatNumber(stolen)}${outOf} дословно совпадают с вопросами чужих паков, выложенных раньше — `
			+ `больше всего с «${top?.name ?? '—'}». Про пак целиком это не говорит ничего: `
			+ `своего в нём ${100 - percent}%. ${caveat}`
		: partial
			? `${percent}% вопросов этого пака уже стояли в чужих паках, выложенных раньше — `
				+ `больше всего в «${top?.name ?? '—'}». Остальное у пака своё. `
				+ `Чужих вопросов — ${formatNumber(stolen)}${outOf}. ${where}. ${caveat}`
			: compiled
				? `${percent}% вопросов этого пака уже стояли в чужих паках, выложенных раньше. `
					+ (top ? `Больше всего — в «${top.name}»: ${top.n} ${plural(top.n, 'вопрос', 'вопроса', 'вопросов')}. ` : '')
					+ `Всего чужих — ${formatNumber(stolen)}${outOf}. ${where}. ${caveat}`
				: `${percent}% вопросов этого пака уже стояли в паке «${top?.name ?? '—'}», выложенном раньше. `
					+ `Чужих вопросов — ${formatNumber(stolen)}${outOf}. ${caveat}. Нажмите, чтобы открыть тот пак`;
...
function createCard(pack, options = {}) {
	...
	// Длительность медиа — сразу за спецвопросами: оба про то, как пак играется,
	// а не про то, из чего он состоит
	const duration = createDuration(pack);
		...
		for (const round of pack.rounds) {
			...
			if (round.themes.length > 0) {
				...
				round.themes.forEach((theme, index) => {
					...
					link.title = donor
						? `Вопросы этой темы уже стояли в паке «${donor.name}», выложенном раньше`
						: 'Вопросы этой темы уже стояли в паке, выложенном раньше';
