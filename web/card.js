 ...
 * он есть только у жанров («Какой жанр музыки?»), потому что без него полоска
 * с надписями «рок, поп» стояла бы третьей и непонятно про что.
 */
...
function createShares(pack) {
	...
	if (!topics && !contents && !genres) {
		return null;
	}

	const box = element('div', 'shares');

	for (const row of [topics, contents, genres]) {
		if (row) {
			box.append(row);
		}
	}
...
/**
 * Третья полоска: жанры внутри тематики пака. Есть она только у паков, которые
 ...
 * ниоткуда, хотя играются такие паки совсем по-разному. Отсюда и подпись
 * вопросом: полоска отвечает ровно на него.
 *
...
function createGenres(pack) {
	...
	return createShareRow(parts, 'shares__bar--genres', 'вопросов пака', list.question, list.question);
}
