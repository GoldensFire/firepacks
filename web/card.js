...
function createPlagiarismBadge(pack) {
	...
	// Нижняя ступень — не обвинение, а наблюдение: чужого заметная доля, но пак
	// стоит на своём костяке. Ей и тон тише — приглушённый, а не тревожный:
	// «часть тем встречалась раньше» и «этот пак — копия» суть разные
	// утверждения, и выглядеть одинаково они не должны
	const partial = mark.kind === 'partial';
	const compiled = mark.kind === 'compiled';

	// Доля отвечает про темы, а спрашивают про вопросы: «на скольких чужих
	// вопросах тут играют». Число это не выводится из доли никак — в теме
	// бывает и пять вопросов, и семь, — поэтому его считает разбор и везёт
	// готовым (см. plagiarism_questions в src/db.js).
	//
	// Ноль значит «неизвестно», а не «ни одного»: у пака, разобранного до того,
	// как темы стали храниться объектами, числа вопросов нет ни у одной темы.
	// Тогда метка про вопросы молчит — писать «0 чужих вопросов» под меткой
	// «темы встречались раньше» значило бы противоречить самому себе
	...
	const badge = element(top ? 'a' : 'span', `badge badge--clue${partial ? ' badge--clue-soft' : ''}`);

	if (top) {
		badge.href = packHref(top);
	}

	// У нижней ступени доля в подписи стоит, а у верхних — нет, и это не описка.
	// «Копия» либо копия, либо нет, и «96,8%» при ней выглядит точностью, которой
	// у правила нет. А «часть тем» без числа не говорит ничего: четверть это
	// или три четверти — весь вопрос
	const label = partial
		? `Часть тем встречалась раньше · ${percent}%`
		: (compiled ? 'Темы из чужих паков' : 'Темы встречались раньше');
	...
	const questionsNote = stolen > 0
		? `Вопросов в этих темах — ${formatNumber(stolen)}${outOf}. `
		: '';
	...
	badge.title = partial
		? `${percent}% тем этого пака уже стояли в чужих паках, выложенных раньше — `
			+ `больше всего в «${top?.name ?? '—'}». Остальное у пака своё. ${questionsNote}`
			+ `У каждой такой темы своя ссылка в «Подробнее». ${caveat}`
		: compiled
			? `${percent}% тем этого пака уже стояли в чужих паках, выложенных раньше. `
				+ `Больше всего — в «${top.name}»: ${top.n} ${plural(top.n, 'тема', 'темы', 'тем')}. ${questionsNote}`
				+ `У каждой темы своя ссылка в «Подробнее». ${caveat}`
			: `${percent}% тем этого пака уже стояли в паке «${top?.name ?? '—'}», выложенном раньше. `
				+ `${questionsNote}${caveat}. Нажмите, чтобы открыть тот пак`;
