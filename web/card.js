 ...
 *
 * Спрашивается это ровно в одном месте — в ярлыке пака, где на вид отводится
 * одно слово в скобках. Полоска же ни у кого ничего не спрашивает: она рисует
 * каждый вид его собственной величиной (см. otherParts).
 */
...
function createBadges(pack) {
	...
	// …но говорить это вправе только тот вид, который в паке и вправду главный.
	// Проверок две, и обе нужны. Первая: само «прочее» должно быть крупнейшим
	// куском пака — иначе выходит «Солянка (Искусство)» у пака 3305, где игр 45%,
	// аниме 30%, а искусства 14%: назван третий по величине. Вторая: вид должен
	// занимать «прочее» почти целиком — иначе выходит «Прочее: география» у пака
	// 502, где прочего 64%, а географии из них 10%.
	//
	// Не прошло ни то, ни другое — ярлык остаётся коротким: «Солянка». Это
	// честный ответ, и подробности про виды никуда не деваются: они стоят
	// в подсказке и отдельными кусками верхней полоски (см. otherParts).
	const otherShare = pack.topicShares?.other ?? 0;
	const leads = Object.values(pack.topicShares ?? {}).every(share => share <= otherShare);
	const topKind = (pack.otherKinds ?? [])[0];
	const named = kinds && leads && topKind && topKind.share >= otherShare * DOMINANT_KIND ? kinds : '';
	...
	if (pack.primaryTopic && !(subject && pack.primaryTopic === 'mixed')) {
		...
		badges.append(createTopicBadge(
			...
			universe
				...
				: pack.primaryTopic === 'mixed'
					? `Ни одна тематика не набрала ${Math.round(facets.topicThreshold * 100)}% вопросов`
						// В подсказке виды названы всегда, даже когда в ярлык они
						// не попали, — и названы тем, чем они являются: не «про это
						// весь пак», а «вот что нашлось в его прочем»
			...
			universe ? `${info.packName} (${capitalize(universe.name)})` : named ? `${info.packName} (${capitalize(named)})` : null,
		));
	}
